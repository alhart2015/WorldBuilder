...
Menu optionMenu(-200, -200, -15, 100);
...
void idle( void ){
    ...
    for (int i = 0; i < tapObjects.size(); i++) {
        ...
        //        if (i != selected) {
        //            while (!tapObjects[i].compColor()) {
        //                tapObjects[i].r -= .01;
        //                tapObjects[i].g -= .01;
        //                tapObjects[i].b -= .01;
        //            }
        //        }
        //        std::cout << "red: " << tapObjects[0].r << " green: " << tapObjects[0].g << " blue: " << tapObjects[0].b << " selected: " << selected << std::endl;
    }
    
    
    
    if (tapped) {
        //std::cout << tapPos.x << " " << tapPos.y << " " << tapPos.z << std::endl;
    }
    
    if (menu_open) {
        // Only worry about selecting a menu item if you haven't already
        Vector fingers = box.normalizePoint(hand.fingers()[1].tipPosition());
        for (int i = 0; i < menuItems.size(); i++) {
            if (menuItems[i].checkInteraction(fingers)) {
                menuItems[i].r = 1;
                menuItems[i].g = 1;
                menuItems[i].b = 1;
                menu_selected = i;
